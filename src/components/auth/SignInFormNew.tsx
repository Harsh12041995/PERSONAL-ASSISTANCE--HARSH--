...
export default function SignInFormNew() {
  ...
  // Subtle full-page parallax: the cosmic scene drifts with the cursor.
  useEffect(() => {
  ...
  return (
    <div ref={rootRef} style={{ position: "relative", width: "100vw", minHeight: "100vh", overflow: "hidden", background: "#04030c", fontFamily: "'Manrope', system-ui, sans-serif", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <style>{KEYFRAMES}</style>

      {/* Full-page cosmic-lion video */}
      <video
        ref={videoRef}
        src={LION_VIDEO}
        poster={LION_POSTER}
        autoPlay loop muted playsInline
        style={{ position: "fixed", inset: 0, width: "100%", height: "100%", objectFit: "cover", transform: "scale(1.16)", willChange: "transform", zIndex: 0 }}
      />
      {/* Depth: vignette + gentle darkening so the card pops */}
      <div style={{ position: "fixed", inset: 0, zIndex: 1, pointerEvents: "none", background: "radial-gradient(120% 90% at 50% 45%, rgba(4,3,12,0) 34%, rgba(4,3,12,0.55) 78%, rgba(4,3,12,0.85) 100%)" }} />
      <div style={{ position: "fixed", inset: 0, zIndex: 1, pointerEvents: "none", background: "linear-gradient(180deg, rgba(4,3,12,0.45) 0%, rgba(4,3,12,0) 22%, rgba(4,3,12,0) 72%, rgba(4,3,12,0.5) 100%)" }} />

      {/* Brand mark */}
      <div style={{ position: "fixed", top: 28, left: 32, zIndex: 3, display: "flex", alignItems: "center", gap: 13 }}>
      ...
      <div style={{ position: "relative", zIndex: 2, padding: "88px 20px 40px", width: "100%", display: "flex", justifyContent: "center" }} onMouseMove={onCardMove} onMouseLeave={onCardLeave}>
        <div ref={cardRef} style={{ width: 430, maxWidth: "100%", borderRadius: 28, background: "rgba(255,255,255,0.14)", backdropFilter: "blur(30px) saturate(150%)", WebkitBackdropFilter: "blur(30px) saturate(150%)", border: "1px solid rgba(255,255,255,0.35)", boxShadow: "0 40px 100px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.5)", padding: "34px clamp(22px,4vw,40px) 28px", transition: "transform 0.2s ease", willChange: "transform", animation: "li-cardIn 0.8s cubic-bezier(0.22,1,0.36,1) both" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ width: 60, height: 60, borderRadius: 17, background: "linear-gradient(135deg,#0fce8e,#2f9df5)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 22, boxShadow: "0 16px 34px rgba(24,165,190,0.5)" }}>PA</div>
            <h2 style={{ margin: "16px 0 0", fontSize: 27, fontWeight: 800, color: "#fff", letterSpacing: "-0.4px", lineHeight: 1.2, textShadow: "0 2px 20px rgba(0,0,0,0.4)" }}>Welcome back</h2>
            <p style={{ margin: "4px 0 0", fontSize: 14.5, color: "rgba(255,255,255,0.82)", fontWeight: 500 }}>Sign in to continue where you left off</p>
          </div>
...
const inputStyle: React.CSSProperties = { position: "absolute", inset: 0, width: "100%", height: "100%", border: "none", outline: "none", background: "transparent", padding: "0 16px 0 46px", fontSize: 15, color: "#0f1424", fontWeight: 500 };
