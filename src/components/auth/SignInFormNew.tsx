...
import AuthCosmicShell, { FONT, authLabel, authErr, authBox, authPrimaryBtn, authSocialBtn } from "./AuthCosmicShell";
...
export default function SignInFormNew() {
  ...
  const inputEl: React.CSSProperties = { position: "absolute", inset: 0, width: "100%", height: "100%", border: "none", outline: "none", background: "transparent", padding: "0 16px 0 46px", fontSize: 15, color: "#0f1424", fontWeight: 500, fontFamily: FONT };
  const iconStyle: React.CSSProperties = { position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)" };

  return (
    <AuthCosmicShell
      footer={
        <p style={{ margin: 0, textAlign: "center", fontSize: 12.5, fontStyle: "italic", color: "rgba(255,255,255,0.62)", lineHeight: 1.6, textShadow: "0 2px 10px rgba(0,0,0,0.6)" }}>
    ...
    >
      ...
      <form onSubmit={handleLogin}>
        <div style={{ marginTop: 26 }}>
          ...
          <div style={authBox(focus === "email", "#6ea3f7", !!errors.email)}>
            ...
            <input id="email" type="email" value={email} onChange={(e) => { setEmail(e.target.value); if (errors.email) setErrors((p) => ({ ...p, email: "" })); }} onFocus={() => setFocus("email")} onBlur={() => setFocus(null)} placeholder="your@email.com" autoComplete="email" style={inputEl} />
          ...
          {errors.email && <p style={authErr}>{errors.email}</p>}
        </div>

        <div style={{ marginTop: 16 }}>
          <label htmlFor="password" style={authLabel}>PASSWORD</label>
          <div style={authBox(focus === "pw", "#8f7cf5", !!errors.password)}>
            ...
            <input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); if (errors.password) setErrors((p) => ({ ...p, password: "" })); }} onFocus={() => setFocus("pw")} onBlur={() => setFocus(null)} placeholder="Enter your password" autoComplete="current-password" style={{ ...inputEl, padding: "0 70px 0 46px" }} />
          ...
          {errors.password && <p style={authErr}>{errors.password}</p>}
        </div>
        ...
        <button type="submit" disabled={sign !== "idle"} style={{ ...authPrimaryBtn, height: 54, marginTop: 22, cursor: sign === "idle" ? "pointer" : "default", opacity: sign === "loading" ? 0.92 : 1 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 14, marginTop: 18 }}>
        <button type="button" aria-label="Continue with Google" onClick={() => social("Google")} style={authSocialBtn}>
        ...
        <button type="button" aria-label="Continue with Apple" onClick={() => social("Apple")} style={authSocialBtn}>
        ...
        <button type="button" aria-label="Continue with Microsoft" onClick={() => social("Microsoft")} style={authSocialBtn}>
        ...
        <button type="button" aria-label="Continue with GitHub" onClick={() => social("GitHub")} style={authSocialBtn}>
