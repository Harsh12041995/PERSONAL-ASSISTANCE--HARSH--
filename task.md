...
- [x] **5.4 Daily Rituals (morning/evening/weekly).** `DailyRitual` model + endpoints; guided morning-plan (top-3 priorities + today's tasks/events), evening reflection (wins/gratitude/tomorrow), weekly review aggregation. *Accept:* setting a morning plan persists; evening reflection saves; weekly view aggregates the week. ✅ `DailyRitual` (unique per user/day) + `/rituals/:date` (get w/ today's task+event context, upsert-merge), `/rituals/weekly` aggregation. `RitualsPage` (`/rituals` + sidebar): morning plan (intention + top-3 + tap-to-pull tasks), evening reflection (wins/gratitude/tomorrow + emoji rating), weekly review card. 13/13 harness PASS.
